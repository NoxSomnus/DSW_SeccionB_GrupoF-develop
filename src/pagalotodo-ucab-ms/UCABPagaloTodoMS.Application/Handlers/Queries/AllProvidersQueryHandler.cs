    ...
    internal class AllProvidersQueryHandler : IRequestHandler<AllProvidersQuery, List<AllProvidersResponse>>
    {
        ...
        private async Task<List<AllProvidersResponse>> HandleAsync(AllProvidersQuery request)
        {
            ...
            try
            {
                ...
                if (result.Count() == 0)
                {
                    throw new AllDataNotFoundException("Error: No hay proveedores de servicio registrados");
                }
                return await result.ToListAsync();
            }
            catch (AllDataNotFoundException ex)
            {
                _logger.LogError(ex, "Error: No hay proveedores de servicio registrados", ex.Message);
                throw;
            }
            catch (Exception ex)
            {
