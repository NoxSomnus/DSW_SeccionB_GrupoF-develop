        ...
        private async Task<Guid> HandleAsync(AddServiceCommand request)
		{
			...
			try
			{
				...
				if (!(user is ProviderEntity provider)) {
					throw new UserIsNotProviderException("Error: El usuario debe ser un provvedor de servicios para poder registrar un servicio");
				}
                ...
                if (consult >0)
				{
                    throw new DataAlreadyExistException("Error: Ya hay un servicio registrado con este nombre");
                }
            ...
            {
                _logger.LogError(ex,"Registrar servicio fallido: No se encontró el usuario",ex.Message);
                transaccion.Rollback();
                throw;
            }
            catch (DataAlreadyExistException ex)
            {
                _logger.LogError(ex, "Registrar servicio fallido: Ya hay un servicio registrado con este nombre", ex.Message);
                transaccion.Rollback();
                throw;
            }
            catch (Exception ex)
