    ...
    internal class ServiceDeleteCommandHandler : IRequestHandler<ServiceDeleteCommand, ServiceDeleteResponse>
    {
        ...
        private async Task<ServiceDeleteResponse> HandleAsync(ServiceDeleteCommand request)
        {
            ...
            try
            {
                ...
                if (service == null)
                {
                    throw new ServiceNotFoundException("Error: El servicio no existe o el usuario no tiene este servicio");
                }
            ...
            {
                _logger.LogError(ex, "Eliminar servicio fallido: No existe el servicio", ex.Message);
