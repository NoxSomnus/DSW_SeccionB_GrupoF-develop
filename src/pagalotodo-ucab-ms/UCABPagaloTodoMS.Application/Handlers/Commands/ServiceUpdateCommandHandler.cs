...
namespace UCABPagaloTodoMS.Application.Handlers.Commands
{
    ...
    public class ServiceUpdateCommandHandler : IRequestHandler<ServiceUpdateCommand, ServiceUpdateResponse>
    {
        ...
        private async Task<ServiceUpdateResponse> HandleAsync(ServiceUpdateCommand request)
        {
            ...
            {
                _logger.LogError(ex,"Actualización fallida: No se encontró el usuario", ex.Message);
                transaccion.Rollback();
                throw;
            }
            catch (UserIsNotProviderException ex)
            {
                _logger.LogError(ex, "Actualización fallida: El usuario no es un proveedor", ex.Message);
                transaccion.Rollback();
                throw;
            }
            catch (ServiceNotFoundException ex)
            {
                _logger.LogError(ex, "Actualización fallida: El proveedor no tiene este servicio", ex.Message);
                transaccion.Rollback();
                throw;
            }
            catch (Exception ex)
