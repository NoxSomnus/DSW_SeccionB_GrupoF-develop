    ...
    public class ChangePasswordCommandHandler: IRequestHandler<ChangePasswordCommand,RecoveryPasswordResponse>
    {
        ...
        private async Task<RecoveryPasswordResponse> HandleAsync(ChangePasswordCommand request)
        {
            ...
            try
            {
                ...
                {
                    throw new ChangePasswordException("Update fallida: campos null");
                }
                else if (request.Password.Password != request.Password.VerifyPassword)// verifica que los campos de la nueva clave coincidan entre si
                {
                    ...
                    throw new ChangePasswordException("Update fallida: Las contraseñas no son iguales");
                }
                else
                {
            ...
            {
                _logger.LogError(ex, "Error: algunos campos están vacíos o llenados de forma incorrecta", ex.Message);
                throw;
            }
            catch (UserNotFoundException ex)
            {
                _logger.LogError(ex, "Error: El usuario no existe", ex.Message);
                throw;
            }
            catch (Exception ex)
            {
