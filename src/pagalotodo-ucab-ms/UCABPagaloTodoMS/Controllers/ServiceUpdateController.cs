...
namespace UCABPagaloTodoMS.Controllers
{
    ...
    public class ServiceUpdateController : BaseController<ServiceUpdateController>
    {
        ...
        public async Task<ActionResult> update([FromBody] ServiceUpdateRequest request)
        {
            ...
            {
                _logger.LogError(ex, "Ocurrió un error al actualizar el servicio: El usuario no es un proveedor", ex.Message);
                return BadRequest(ex.Message);
            }
            catch (ServiceNotFoundException ex)
            {
                _logger.LogError(ex, "Ocurrió un error al actualizar el servicio: El proveedor no tiene este servicio", ex.Message);
                return NotFound(ex.Message);
            }
            catch (Exception ex)
            {
