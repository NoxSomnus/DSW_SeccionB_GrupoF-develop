...
namespace UCABPagaloTodoMS.Controllers
{
    ...
    public class BillQueryController : BaseController<BillQueryController>
    {
        ...
        public async Task<ActionResult> AllBills()
        {
            ...
            {
                _logger.LogError("Ocurrio un error en la consulta de las facturas: No hay facturas registradas" + ex);
        ...
        public async Task<ActionResult> byUserId([FromQuery] Guid UserId)
        {
            ...
            {
                _logger.LogError("Ocurrio un error en la consulta de las facturas: No se encontró el usuario" + ex);
                return NotFound(ex.Message);
            }
            catch (BillsNotFoundException ex)
            {
                _logger.LogError("Ocurrio un error en la consulta de las facturas: No hay facturas asociada a este usuario" + ex);
                return NotFound(ex.Message);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult> byServiceId([FromQuery] Guid ServiceId)
        {
            ...
            {
                _logger.LogError("Ocurrio un error en la consulta de las facturas: No se encontró el servicio" + ex);
                return NotFound(ex.Message);
            }
            catch (BillsNotFoundException ex)
            {
                _logger.LogError("Ocurrio un error en la consulta de las facturas: No hay facturas asociadas a este servicio" + ex);
                return NotFound(ex.Message);
            }
            catch (Exception ex)
            {
