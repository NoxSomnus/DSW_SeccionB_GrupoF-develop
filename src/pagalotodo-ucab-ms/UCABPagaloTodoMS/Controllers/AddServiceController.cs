...
namespace UCABPagaloTodoMS.Controllers
{
	...
	public class AddServiceController : BaseController<AddServiceController>				
	{
		...
		public async Task<ActionResult> AddService([FromBody] AddServiceRequest request)
		{
            ...
            {
                _logger.LogError(ex, "Ha ocurrido un error: El usuario debe ser un proveedor de servicios para poder registrar un servicio", ex.Message);
                return BadRequest(ex.Message);	
            }
            catch (DataAlreadyExistException ex)
            {
                _logger.LogError(ex, "Ha ocurrido un error: Ya hay un servicio registrado con este nombre", ex.Message);
                return BadRequest(ex.Message);
            }
            catch (Exception ex)
			{
